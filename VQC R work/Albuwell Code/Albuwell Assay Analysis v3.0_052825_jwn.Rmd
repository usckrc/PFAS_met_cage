...
data <- inner_join(df.meta2, creatinine_table, by = c( "ID" = "Sample"))


#convert grams to dL for urine
data$Urine_dL <- data$UV * 0.01

tidy_data_solved_dil_meta <- na.omit(data)
  
#creatinine mass
tidy_data_solved_dil_meta$Creatinine_Mass <- tidy_data_solved_dil_meta$Urine_dL * tidy_data_solved_dil_meta$Cre_Concentration
...
ggplot(tidy_data_solved_dil_meta, aes(x = Treatment, y = Creatinine_Mass, color = Timepoint)) +
  geom_point(position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.6), size = 3) +
  labs(title = "Creatinine Mass at Baseline and Endpoint by Treatment Group",
       x = "Treatment Group",
       y = "Creatinine Mass") +
  theme_minimal() +
  scale_color_manual(values = c("Baseline" = "#1f77b4", "Final" = "#ff7f0e"))

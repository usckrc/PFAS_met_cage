...
##b.Calcuate back the latter values based on the fit equation
...
##c. Graph the sample data on the solved curve split by dilution factor
...
p + 
  geom_point(
    data = df_sample_test %>% filter(!is.na(Solved), !is.na(Value2), !is.na(Dilution)),
    aes(x = Solved, y = Value2, color = as.character(Dilution)),
    size = 3
  ) +
  labs(color = "Dilution")
...
##d. Calculate with Dilution Factor
...
ggplot(ACR_meta, aes(x = `Timepoint Cre`, y = ACR_mg_g, color = as.character(Treatment))) +
  geom_point(size = 3) +
  theme_minimal() +

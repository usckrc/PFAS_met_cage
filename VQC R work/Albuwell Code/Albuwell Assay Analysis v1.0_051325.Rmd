---
title: "Albuwell Assay Analysis 2.0"
...
dilution <- read.xlsx(here("VQC R work", "Albuwell Data Sheets", "Long_PFAS_Albuwell_051325.xlsx"), detectDates = TRUE)


subset <- dilution[18:25, 2:5]
...
dilution_sample <- read.xlsx(here("VQC R work", "Albuwell Data Sheets", "Long_PFAS_Albuwell_Sample_051325.xlsx"), detectDates = TRUE)

tidy_data <- tidy %>%
  left_join(dilution_sample, by = c("Cell" = "Cell")) 
...
p + geom_point(data = df_sample_test, aes(x = Solved, y = Value2), color = as.character(df_sample_test$Dilution), size = 3)

# If Dilution is greater than 10 then write (Above), if the not write OK

df_sample_test <- df_sample_test %>%
  mutate(Dilution_calc = ifelse(Solved > 10, "Above", "OK"))
...
```{r, echo=TRUE, warning=FALSE, error=FALSE, message=FALSE,  fig.width=20, fig.height=5}
...
df_creatinine <- read.xlsx(here("Dataset (241122)", "241114_Creatinine_NK.xlsx"))

subset_pre <- df_creatinine[19:26, 2:9]

# rename the columns to 1-12
colnames(subset_pre) <- c("Row", 1:7)
...
df_creatinine_post <- read.xlsx(here("Dataset (241122)", "241114_Creatinine+Acid_NK.xlsx"))

subset_post <- df_creatinine_post[19:26, 2:9]

# rename the columns to 1-12
colnames(subset_post) <- c("Row", 1:7)
...
df.Sample <- read.xlsx(here("Dataset (241122)", "241114 Creatinine Sample Info.xlsx"))

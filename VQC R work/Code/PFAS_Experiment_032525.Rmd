...
colnames(VEH_BW_plot)

VEH_BW_plot_2 <- VEH_BW_plot %>% left_join(PFAS_meta, by = c("ID" = "ID")) %>%
                        rename(Timepoint = X1)

VEH_BW_plot_2$Timepoint <- factor(VEH_BW_plot_2$Timepoint, levels = c("D-2", "D-1", "D0", "D1"))
VEH_BW_plot_2$Drug <- factor(VEH_BW_plot_2$Drug, levels = c("Vehicle", "PFAS"))


VEH_BW_plot_3 <- VEH_BW_plot_2 %>%
...
VEH_BW_plot_5 <- VEH_BW_plot_4 %>%
  group_by(Drug) %>%
  mutate(NormalizedMean = Mean / first(Mean)) %>%
  mutate(NormalizedSD = SD / first(Mean))

VEH_BW_plot_5$NormDown <- VEH_BW_plot_5$NormalizedMean - VEH_BW_plot_5$NormalizedSD
VEH_BW_plot_5$NormUp <- VEH_BW_plot_5$NormalizedMean + VEH_BW_plot_5$NormalizedSD

VEH_BW_plot_5$Timepoint <- factor(VEH_BW_plot_5$Timepoint, levels = c("D-2", "D-1", "D0", "D1"))
VEH_BW_plot_5$Drug <- factor(VEH_BW_plot_5$Drug, levels = c("Vehicle", "PFAS"))

# Example plot
ggplot(data = VEH_BW_plot_5, aes(x = Timepoint, ymin = NormDown, ymax = NormUp, y = NormalizedMean, color = Drug, group = Drug, fill = Drug)) + 
